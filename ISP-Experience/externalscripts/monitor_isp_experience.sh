#!/usr/bin/env bash
# ==============================================================================
# monitor_isp_experience.sh - Coleta de Experiência, MTR Trace e Detecção de Rota
# ==============================================================================
...
# PROTEÇÃO ANTI-FALSO POSITIVO (Burlar bloqueio de Ping ICMP em Bancos/CDNs):
# Se o site bloqueia Ping ICMP (100% loss) MAS respondeu TCP Handshake ou HTTP corretamente,
# não devemos considerar o serviço offline!
IS_ONLINE=1
if [ "$HTTP_CODE" -eq 0 ] && [ $(awk "BEGIN {print ($TCP_HANDSHAKE_MS == 0) ? 1 : 0}") -eq 1 ] && [ "$ICMP_LOSS" -eq 100 ]; then
    IS_ONLINE=0
fi

# Se respondeu HTTP ou TCP, normalizamos ICMP_LOSS para 0 caso seja bloqueio de firewall
...
# Procura assinaturas típicas de PTT/IX.br ou saltos curtos de CDN direta
...
# 6. Saída JSON formatada com MTR Trace incluso
cat <<EOF
{
  "status": $IS_ONLINE,
  "id": "$ID",
