...
│   └── Template.xml
└── externalscripts/
    ├── dns_check.sh       ← coleta de métricas (DIG e NSLOOKUP)
    └── dns_discover.py    ← discovery LLD (gera JSON de combinações)
```
...
```bash
cp DNS-Monitor/externalscripts/dns_check.sh /usr/lib/zabbix/externalscripts/
cp DNS-Monitor/externalscripts/dns_discover.py /usr/lib/zabbix/externalscripts/
chmod +x /usr/lib/zabbix/externalscripts/dns_check.sh
chmod +x /usr/lib/zabbix/externalscripts/dns_discover.py
chown zabbix:zabbix /usr/lib/zabbix/externalscripts/dns_check.sh
chown zabbix:zabbix /usr/lib/zabbix/externalscripts/dns_discover.py
```
...
/usr/lib/zabbix/externalscripts/dns_check.sh 8.8.8.8 google.com A dig time

# Deve retornar 1 (sucesso)
/usr/lib/zabbix/externalscripts/dns_check.sh 8.8.8.8 google.com A dig status

# Deve retornar NOERROR
/usr/lib/zabbix/externalscripts/dns_check.sh 8.8.8.8 google.com A dig rcode

# Deve retornar JSON com as combinações
/usr/lib/zabbix/externalscripts/dns_discover.py "8.8.8.8,1.1.1.1" "google.com" "A"
```
...
Buscar por `Template DNS Monitor` e salvar.
...
|---|---|---|
| `{$DNS_SERVERS}` | `8.8.8.8,8.8.4.4,1.1.1.1` | Servidores DNS a monitorar, separados por vírgula |
| `{$DNS_DOMAINS}` | `google.com` | Domínios de teste, separados por vírgula |
| `{$DNS_TYPES}` | `A` | Tipos de registro (`A`, `AAAA`, `MX`, `NS`...), separados por vírgula |
| `{$DNS_TIMEOUT}` | `5` | Timeout em segundos para cada consulta |
| `{$DNS_WARN_TIME}` | `200` | Limiar WARNING de tempo de resposta (ms) |
| `{$DNS_CRIT_TIME}` | `500` | Limiar CRITICAL de tempo de resposta (ms) |

**Exemplo para monitorar DNS interno e externo:**

```
{$DNS_SERVERS}  →  192.168.1.1,8.8.8.8,1.1.1.1
{$DNS_DOMAINS}  →  google.com,seudominio.com.br
{$DNS_TYPES}    →  A,MX
```
...
O script `dns_discover.py` recebe as macros como argumentos e retorna um JSON com todas as combinações:
...
|---|---|---|
| DNS DIG lento CRITICAL | HIGH | Tempo DIG > `{$DNS_CRIT_TIME}` ms |
| DNS DIG lento WARNING | WARNING | Tempo DIG > `{$DNS_WARN_TIME}` ms e ≤ CRITICAL |
| DNS DIG falhou | AVERAGE | Status DIG = 0 |
| DNS DIG RCODE anormal | WARNING | RCODE ≠ `NOERROR` |
| DNS NSLOOKUP lento | HIGH | Tempo NSLOOKUP > `{$DNS_CRIT_TIME}` ms |
...
- Testar o script manualmente como usuário `zabbix`: `sudo -u zabbix /usr/lib/zabbix/externalscripts/dns_discover.py "8.8.8.8" "google.com" "A"`
- Verificar o log do servidor: `tail -f /var/log/zabbix/zabbix_server.log | grep dns`

**Itens retornam `9999` ms:**
- O servidor DNS não respondeu dentro do timeout (`{$DNS_TIMEOUT}`)
...
- Aumentar `{$DNS_TIMEOUT}` se a rede tiver alta latência
